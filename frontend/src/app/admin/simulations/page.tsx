...
export default function ImporViaAdminSimulations() {
  ...
  const fetchSimulations = () => {
    ...
    apiClient.get(`/admin/simulations/?${params.toString()}`)
      .then(res => {
        setSimulations(res.data.results || [])
  ...
  const fetchStats = () => {
    apiClient.get(`/admin/analytics/`)
      .then(res => {
        setStats({
          total: res.data.total_simulations || 0,
          completed: res.data.simulations_completed || 0,
          failed: res.data.simulations_failed || 0,
        })
      })
      .catch(console.error)
  }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 font-medium mt-1">Supervisez toutes les simulations de dédouanement du système en temps réel.</p>
          </div>
          <div className="flex items-center gap-2">
            <span className="bg-blue-50 text-blue-700 border border-blue-200 px-3 py-1.5 rounded-xl text-sm font-bold">
              {stats.total.toLocaleString("fr-FR")} simulations
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { label: "Simulations (Total)", value: stats.total.toLocaleString("fr-FR"), icon: <Activity className="w-5 h-5 text-blue-600" />, bg: "bg-blue-50" },
            { label: "Taux de succès global", value: `${successRate}%`, icon: <TrendingUp className="w-5 h-5 text-emerald-600" />, bg: "bg-emerald-50" },
            { label: "Erreurs détectées", value: stats.failed.toLocaleString("fr-FR"), icon: <AlertCircle className="w-5 h-5 text-red-600" />, bg: "bg-red-50" },
          ].map((kpi, i) => (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase tracking-wider">
                <tr>
                  ...
                  <th className="py-4 px-6 whitespace-nowrap">Utilisateur & Entreprise</th>
              ...
              <tbody className="text-sm">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="text-center py-8 text-slate-500 font-medium">Aucune simulation trouvée.</td>
                  </tr>
                ) : simulations.map((sim, i) => {
                  const s = statusConfig[sim.status === "COMPLETED" ? "completed" : sim.status === "FAILED" ? "error" : "pending"] || statusConfig.pending
                  return (
                    <tr key={sim.id || i} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      <td className="py-4 px-6 font-mono font-bold text-blue-600 text-xs">{sim.reference}</td>
                      <td className="py-4 px-6 whitespace-nowrap">
                        <div className="font-bold text-slate-900">{sim.created_by_email}</div>
                        <div className="text-slate-500 text-xs">{sim.company_name}</div>
                      </td>
                      <td className="py-4 px-6 font-mono font-bold text-slate-700">{sim.hs_code_display}</td>
          ...
          <div className="bg-slate-50 border-t border-slate-200 p-4 flex items-center justify-between text-sm font-medium text-slate-500">
            <span>Affichage de {(page - 1) * 20 + 1} à {Math.min(page * 20, count)} sur {count} simulations</span>
